...
const tempDir = mkdtempSync(path.join(tmpdir(), 'hiwords-japanese-matcher-runtime-'))
...
try {
    await esbuild.build({
        entryPoints: [path.join(repoRoot, 'tests/japanese-morphology/matcher-runtime.test.ts')],
        outfile: bundlePath,
        bundle: true,
        platform: 'node',
        format: 'esm',
